...
const boton = { width:"100%", padding:"12px", background:"#111", color:"#fff", border:"none", borderRadius:4, fontSize:14, fontWeight:600, cursor:"pointer" };
const tituloSeccion = { fontSize:22,fontWeight:600,letterSpacing:"-0.02em",color:"#111",margin:"0 0 8px" };
const subtitulo = { fontSize:15,color:"#787774",lineHeight:1.6,margin:"0 0 28px" };
...
export default function Dashboard() {
  ...
  const [barrioId,setBarrio]=useState(20);
  const [diaSemana,setDia]=useState(diaActual);
  const [resultado,setResultado]=useState(null);
  const [errorMsg,setError]=useState("");
  const [todasPred,setTodasPred]=useState(null);
  const [sliderVal,setSliderVal]=useState(horaActual);

  useEffect(() => {
    prediccionesTodas(hora,diaSemana).then(setTodasPred).catch(()=>{});
  }, [hora,diaSemana]);

  const consultar = async () => {
    setError("");
    try { setResultado(await predecir(hora,barrioId,diaSemana)); }
    catch(ex) { setError(ex.message); }
  };

  const nivel = resultado ? coloresRiesgo[resultado.nivel_riesgo] : null;

  return (
    <div style={{display:"grid",gap:28}}>

      <header>
        <h1 style={tituloSeccion}>Centinela</h1>
        <p style={subtitulo}>
      ...
      <div style={{...card,padding:0,position:"relative"}}>
        <MapView predicciones={todasPred} barrioId={barrioId} onSelect={(id)=>{setBarrio(id);consultar();}} />
      ...
      <div style={{...card,padding:28}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
          <div><label style={label}>Hora</label><input type="number" min={0} max={23} value={hora} onChange={e=>{const v=+e.target.value; setHora(v); setSliderVal(v);}} style={field} /></div>
          <div><label style={label}>Dia</label><select value={diaSemana} onChange={e=>setDia(+e.target.value)} style={field}>{DIAS_SEMANA.map((n,i)=><option key={i} value={i}>{n}</option>)}</select></div>
          <div><label style={label}>Barrio</label><select value={barrioId} onChange={e=>setBarrio(+e.target.value)} style={field}>{Object.entries(BARRIOS_CABA).map(([id,n])=><option key={id} value={id}>{n}</option>)}</select></div>
        </div>

        {resultado && (
          <div style={{marginBottom:16}}>
            <label style={{...label,marginBottom:8}}>Evolucion del riesgo en {BARRIOS_CABA[barrioId]}</label>
            <input type="range" min={0} max={23} value={sliderVal}
              onChange={e=>{const v=+e.target.value; setSliderVal(v); setHora(v); consultar();}}
        ...
        <button onClick={consultar} style={boton}>Ver prediccion</button>
        {errorMsg && <p style={{color:"#9F2F2D",fontSize:13,marginTop:12}}>{errorMsg}</p>}
        {resultado && (
          <div style={{marginTop:16,background:nivel.bg,border:`1px solid ${nivel.border}`,borderRadius:6,padding:20,textAlign:"center"}}>
            <p style={{fontSize:40,fontWeight:700,margin:0,color:nivel.text,letterSpacing:"-0.02em"}}>{(resultado.probabilidad*100).toFixed(1)}%</p>
            <p style={{fontSize:14,fontWeight:500,color:nivel.text,margin:"4px 0"}}>Nivel {nivel.label} &middot; {BARRIOS_CABA[barrioId]}</p>
            {resultado.desde_cache && <p style={{fontSize:11,color:"#787774",margin:0}}>resultado en cache</p>}
          </div>
        )}
      </div>

      <div style={{...card,padding:28}}>
        <h2 style={{...tituloSeccion,fontSize:16,marginBottom:12}}>Como funciona</h2>

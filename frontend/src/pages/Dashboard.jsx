...
const risk = (v) => v?{bg:"#ffebee",border:"#ef5350",color:"#c62828",label:"ALTO"}:{bg:"#e8f5e9",border:"#66bb6a",color:"#2e7d32",label:"BAJO"};
  ...
  return (
    <div style={{display:"grid",gap:20}}>
      ...
      <div style={card}>
        ...
        {r && (
          <div style={{marginTop:20,background:risk(r.alto_riesgo).bg,border:`2px solid ${risk(r.alto_riesgo).border}`,borderRadius:12,padding:24,textAlign:"center"}}>
            <p style={{fontSize:48,fontWeight:800,margin:0,color:risk(r.alto_riesgo).color}}>{(r.probabilidad*100).toFixed(1)}%</p>
            <p style={{fontSize:18,fontWeight:600,color:risk(r.alto_riesgo).color,margin:"4px 0"}}>Riesgo {risk(r.alto_riesgo).label}</p>
            <p style={{fontSize:12,color:"#888"}}>{r.desde_cache?"⚡ caché":"🧠 calculado"}</p>
          </div>
        )}
      </div>
    </div>
